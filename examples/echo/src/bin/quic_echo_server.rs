...
pub struct Handle{
    fut: std::cell::RefCell<Option<Pin<Box<MyTimeoutFuture>>>>,
}
...
impl ClientHelloCallback for Handle {
    fn poll_client_hello(&self, conn: &mut Connection) -> core::task::Poll<Result<(),s2n_tls::error::Error>> {
        println!("Poll handle..");
        ...
        if fut.is_none() {
            fut.replace(Pin::new(Box::new(MyTimeoutFuture::new(Duration::from_secs(5)))));
            Poll::Pending
        }else {
